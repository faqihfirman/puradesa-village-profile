...
export default function EconomicPotentialCard({ potential }: { potential: EconomicPotential }) {
  const IconComponent = sectorIcon(potential.sector);

  return (
    <Link
      href={`/umkm/${potential.slug}`}
    ...
    >
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-secondary-container">
        {potential.imageUrl ? (
          <img src={potential.imageUrl} alt={potential.title} className="h-full w-full object-cover" loading="lazy" />
        ) : (
        ...
        <span className="absolute left-3 top-3 flex items-center gap-1.5 rounded-badge bg-primary px-2.5 py-1 font-display text-xs font-semibold text-on-primary">
          <IconComponent className="h-3.5 w-3.5" /> {potential.sectorLabel}
        </span>
      </div>
      <div className="flex flex-1 flex-col gap-2 p-5">
        <h3 className="font-display text-lg font-semibold text-ink group-hover:text-primary">{potential.title}</h3>
        <p className="line-clamp-2 text-sm text-body">{potential.description}</p>
